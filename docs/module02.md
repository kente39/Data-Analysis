...
평균·중앙값·표준편차 같은 기술통계를 실제 코드로 계산해 봅니다. `describe()`가 무엇을 계산하는지 직접 수동으로 검증하고, IQR·Z-score로 이상치를 탐지하며, 문자열로 위장한 가짜 결측치까지 찾아냅니다.

## 2-1. describe() — 한 방에 보기

::: tip 핵심
`describe()` 하나로 개수·평균·표준편차·사분위수를 한 번에 확인할 수 있습니다. **평균(51.30) > 중앙값(48.80)**이면 오른쪽으로 치우친 분포를 의심할 수 있습니다.
:::
...
평균 - 중앙값이 양수(+2.50)라는 것은 소수의 큰 값이 평균을 끌어올렸다는 뜻입니다. 즉, 오른쪽 꼬리(양의 왜도) 가능성을 시사합니다.
