...
`plant_growth.csv`를 여러 종류의 차트로 그려 봅니다. Matplotlib의 기본 구조부터 Seaborn의 통계 시각화까지, 데이터를 눈으로 탐색하는 방법을 익힙니다. 모든 차트는 실제 5,000행 데이터로 그렸습니다.
...
대각선은 자기 자신과의 상관이라 항상 1.00입니다.  
`prev_height_cm ↔ height_cm`(0.95)는 매우 강한 양의 상관을 보이고, 이를 제외하면 나머지 상관은 대부분 |상관| < 0.1로 약합니다.  
즉 **현재 포함된 수치 변수들 중에서는** 키를 선형으로 예측할 단서가 거의 `prev_height_cm`에 집중되어 있다는 뜻이며, 이것이 6장에서 단순 선형회귀가 의외로 강한 이유로 이어집니다.
...
`hue='species'`로 색을 나누면 **어떤 종이 어느 크기 구간에 몰려 있는지**까지 드러, 첫 EDA에서 분포·관계·군집·이상치를 동시에 훑는 용도로 씁니다.
